...
class MyPlot:
    ...
    #constructor
    def __init__(self,stats,classstats,name,cl,path):
        self.stats = stats
        self.classstats = classstats
        self.name = name
        self.cl = cl
        if self.cl == "one":
            self.plotClass(stats,classstats,len(classstats),name,path)
        if self.cl == "two":
            self.plotClass2(classstats,name,path)
    ...
    def plotClass(self,stats,classstats,c_num,name,path):
        fig = pylab.figure()
    ...
    def plotPer(self,figu,stats,classstats,f_num,c_num,name):
        ...
        li = ('averages (inc.)','averages (cum.)',
              'Brown', 'Clinical', 'Geoquery', 'TREC', 'Wacky')
    ...
    def plotClass2(self,classstats,name,path):
        fig = pylab.figure(figsize=(5,8), dpi=100)
        self.plotPerSim(fig, classstats,name)
        pylab.ioff()
        #to use if needed
        #mytime = '%.2f' % time()
        mytime = ""
    ...
    def plotPerSim(self,figu,classstats,name):
        ...
        ax.grid(False)
        li = ('trend of distribution','open domain','processes','clinical')
        self.plotLegend(ax,li)
